...
Les messages traduisibles vivent dans `src/messages/`, decoupes par domaine (`common`, `doctor`, `help`, puis plus tard `config`, etc.). Ajouter les nouvelles surfaces par lots coherents plutot que melanger traduction et refactor large. `palabre doctor` et `palabre help` sont les premieres commandes migrees vers le dictionnaire FR/EN.
