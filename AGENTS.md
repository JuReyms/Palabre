...
Les messages traduisibles vivent dans `src/messages/`, decoupes par domaine (`common`, `doctor`, puis plus tard `help`, `config`, etc.). Ajouter les nouvelles surfaces par lots coherents plutot que melanger traduction et refactor large. `palabre doctor` est la premiere commande migree vers le dictionnaire FR/EN.
